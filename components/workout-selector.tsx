      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-2 gap-4">
          <Button
            ...
            className={`h-24 rounded-xl border-2 ${selectedDay === "A" ? "border-primary bg-primary/10" : "border-muted"}`}
          >
            <div className="flex flex-col items-center">
              <span className="text-2xl font-bold">A</span>
              {lastWorkoutA && (
                <span className="text-xs text-muted-foreground mt-1">
                  Último: {formatDate(lastWorkoutA.date, true)}
                </span>
              )}
            </div>
          </Button>
          <Button
            ...
            className={`h-24 rounded-xl border-2 ${selectedDay === "B" ? "border-primary bg-primary/10" : "border-muted"}`}
          >
            <div className="flex flex-col items-center">
              <span className="text-2xl font-bold">B</span>
              {lastWorkoutB && (
                <span className="text-xs text-muted-foreground mt-1">
                  Último: {formatDate(lastWorkoutB.date, true)}
                </span>
              )}
            </div>
          </Button>
